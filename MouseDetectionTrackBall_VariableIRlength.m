...
if ispc
    [Fname, Pname] = uigetfile('F:\TrackBallMovie\*Combined.avi','MultiSelect','off');
elseif ismac
...
for i = 1:steps
    ...
    ChR2LaserLumi = s(i).Data(122:139,705:750);
    ChR2LaserLumi = reshape(ChR2LaserLumi,[],1);
    
    ChR2LaserLumi = mean(topkrows(ChR2LaserLumi,30));
    s(i).Laser = LaserLumi;
...
ChR2LaserForPosi = ChR2Laser;
[~,ChR2PeakFrame] = findpeaks(ChR2Laser,'MinPeakDistance',300, 'MinPeakHeight',mean(ChR2Laser)+60);

% % 20190826 added
% figure(102)
% for i = 1:length(ChR2PeakFrame)
%     ChR2PosiXvalue = mean(s(ChR2PeakFrame(i)).Data(122:139,705:750),1);
%     ChR2PosiX = 1:length(ChR2PosiXvalue);
% %     ChR2PosiY = mean(s(ChR2PeakFrame(i)).Data(122:139,705:750),2);
%     ChR2PosiXvalue = movmean(ChR2PosiXvalue,5);
% 
%     figure(102)
%     plot(ChR2PosiXvalue/max(ChR2PosiXvalue))
%     hold on
% 
%     ChR2PosiMax = ChR2PosiX(ChR2PosiXvalue==max(ChR2PosiXvalue));
%     ChR2PosiX2 = ChR2PosiX;
%     ChR2PosiX2(ChR2PosiX2>ChR2PosiMax) = [];
%     ChR2PosiXvalue(ChR2PosiX>max(ChR2PosiX2)) = [];
%     ChR2PosiHalf = ChR2PosiX(ChR2PosiXvalue>9*(max(ChR2PosiXvalue)/10));
%     [~,I] = min(abs(ChR2PosiXvalue-9*(max(ChR2PosiXvalue)/10)));
%     ChR2Posi1 = ChR2PosiX(I);
% %     ChR2Posi1 = min(ChR2PosiHalf(ChR2PosiHalf<ChR2PosiMax & ChR2PosiHalf>ChR2PosiMax-5));
% %     ChR2Posi2 = max(ChR2PosiHalf(ChR2PosiHalf>ChR2PosiMax & ChR2PosiHalf>ChR2PosiMax-5));
% %     plot(mean(ChR2Posi1,ChR2Posi2), max(ChR2PosiXvalue), 'o')
%     plot(ChR2Posi1, ChR2PosiXvalue(ChR2Posi1)/max(ChR2PosiXvalue), 'o')
% %     plot(ChR2PosiX,min(ChR2PosiXvalue(ChR2PosiXvalue>3*(max(ChR2PosiXvalue)/4)), 'o')
%     ChR2PosiX90percent(i) = ChR2Posi1;
% end
% hold off
% ChR2PosiX90percent(ChR2PosiX90percent<6)=[];
% 
% if std(ChR2PosiX90percent)>3
%     MultiLaserPosi = 1;
% else
    MultiLaserPosi = 0;
% end
...
LaserOnsetFrame(LaserDurationFrame < 5) = [];
LaserOffsetFrame(LaserDurationFrame < 5) = [];
LaserDurationFrame(LaserDurationFrame < 5) = [];

LaserTest = zeros(length(Laser),1);
LaserTest(LaserOnsetFrame)=200;
LaserTest(LaserOffsetFrame)=-200;
Laser = cumsum(LaserTest);
...
if length(ChR2LaserDurationFrame) >20
    ChR2LaserOnsetFrame(ChR2LaserDurationFrame < 20) = [];
    ChR2LaserOffsetFrame(ChR2LaserDurationFrame < 20) = [];
    ChR2LaserDurationFrame(ChR2LaserDurationFrame < 20) = [];
end
...
elseif length(ChR2LaserOnsetFrame)==21
    ChR2LaserOnsetFrame(21) = [];
    ChR2LaserOffsetFrame(21) = [];
    ChR2LaserDurationFrame(21) = [];
elseif  length(ChR2LaserOnsetFrame)==16
    ChR2LaserOnsetFrame(16) = [];
    ChR2LaserOffsetFrame(16) = [];
    ChR2LaserDurationFrame(16) = [];
else
    ChR2LaserOnsetFrame(ChR2LaserDurationFrame < 90) = [];
    ChR2LaserOffsetFrame(ChR2LaserDurationFrame < 90) = [];
    ChR2LaserDurationFrame(ChR2LaserDurationFrame < 90) = [];
end
...
d1 = designfilt('bandpassiir','FilterOrder',12, ...
    'HalfPowerFrequency1',0.05,'HalfPowerFrequency2',0.6,'DesignMethod','butter');  %low cutoff = 0.05*(30/2)
...
ControlOnsetFrames = sort(ControlOnsetFrames);

for jj=1:length(ControlOnsetFrames)
    plot([ControlOnsetFrames(jj) ControlOnsetFrames(jj)], [0 500],'k:')
end
...
for i=1:length(ChR2orIR_LaserOnsetFrame)
    ...
    title(['Fr' num2str(ChR2orIR_LaserOnsetFrame(B(i)))])
...
for i=1:length(ControlOnsetFrames)-TrialReduce
    ...
    title(['Fr=' num2str(ControlOnsetFrames(i))])
...
imagesc(s(min(Frames(Eye == min(Eye)))).Data(30:120,30:150)),colormap(gray)
...
txt1 = ['1=control, 2=ChR(+),3=LaserS,4=LaserS+ChR,'];
text(-2,-2,txt1)
txt2 = ['5=LaserM,6=LaserM+ChR,5=LaserL,6=LaserL+ChR2'];
text(-2,-3,txt2)
...
if SaveFig == 1
    ...
    saveas(h1,[Pname fn(1:end-12) 'Alltrial.png'],'png')
    saveas(h3,[Pname fn(1:end-12) 'hist.png'],'png')
